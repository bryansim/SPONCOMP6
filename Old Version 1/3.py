...
print "[Last Person's Best: 1963] Please count backwards from 2023 to 0 in 3-step sequences: "
timestart = time.time()
...
while counter != 0: # while counter is not zero
    while True:
        try: # tests to see if participant's input is a number
            ...
            print "[Last Person's Best: 1963] Only numbers are valid."        
    if counter == number: # if participant types in the correct answer
        print "\n" * 80 # spacer to clear screen
        print "[Last Person's Best: 1963] Next: " 
